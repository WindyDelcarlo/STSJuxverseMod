...
public class TranscriptionTablet extends BaseRelic {
    private static final String NAME = "TranscriptionTablet";
    public static final String ID = makeID(NAME);
    private static final RelicTier RARITY = RelicTier.UNCOMMON;
    private static final LandingSound SOUND = LandingSound.FLAT;


    public TranscriptionTablet() {
        super(ID, NAME, CharRianne.Meta.CARD_COLOR, RARITY, SOUND);
    }

    @Override
    public void onUseCard(AbstractCard card, UseCardAction action){
        if (card.type.equals(AbstractCard.CardType.SKILL) && !card.hasTag(JuxverseMod.CONSTELLATION)){
            addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player,new StarPower(AbstractDungeon.player,1)));
            addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player,new StarlessPower(AbstractDungeon.player,1)));
        }
    }
