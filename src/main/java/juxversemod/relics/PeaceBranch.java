...
import juxversemod.powers.PeacePower;

import static juxversemod.JuxverseMod.makeID;

public class PeaceBranch extends BaseRelic {
    private static final String NAME = "PeaceBranch";
    ...
    public void onMonsterDeath(AbstractMonster m) {
        if (m.currentHealth == 0 && !AbstractDungeon.getMonsters().areMonstersBasicallyDead()) {
            flash();
            addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new PeacePower(AbstractDungeon.player, 1)));
        }
    }
