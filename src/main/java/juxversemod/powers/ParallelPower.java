...
public class ParallelPower extends BasePower {

    public static final String POWER_ID = makeID("ParallelPower");
    private static final PowerType TYPE = PowerType.BUFF;
    private static final boolean TURN_BASED = false;

    public ParallelPower(AbstractCreature owner, int amount){
    ...
    public void onUseCard(AbstractCard card, UseCardAction action) {
        if (!card.purgeOnUse && this.amount > 0){
            ...
            this.amount--;
            if (this.amount == 0){
                addToTop(new RemoveSpecificPowerAction(owner,owner,ParallelPower.POWER_ID));
            }
        }
    }
    ...
    public void atEndOfTurn(boolean isPlayer) {
        addToBot(new RemoveSpecificPowerAction(owner,owner,ParallelPower.POWER_ID));
    }
