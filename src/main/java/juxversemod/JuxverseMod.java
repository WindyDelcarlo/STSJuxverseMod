...
import juxversemod.cards.BaseCard;
import juxversemod.characters.CharRianne;
import juxversemod.events.ForbiddenKnowledgeEvent;
import juxversemod.relics.BaseRelic;
import juxversemod.util.GeneralUtils;
import juxversemod.util.KeywordInfo;
import juxversemod.util.TextureLoader;
...
@SpireInitializer
public class JuxverseMod implements
    ...
    private static String checkResourcesPath() {
        String name = JuxverseMod.class.getName(); //getPackage can be iffy with patching, so class name is used instead.
        ...
        if (!resources.exists()) {
            throw new RuntimeException("\n\tFailed to find resources folder; expected it to be at  \"resources/" + name + "\"." +
                    ...
                    "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n" +
                    "\tat the top of the " + JuxverseMod.class.getSimpleName() + " java file.");
        }
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(JuxverseMod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
    ...
    public void receiveEditCharacters() {
        CharRianne.Meta.registerCharacter();
    }
    ...
    @SpireEnum
    public static AbstractCard.CardTags CONSTELLATION;
}
